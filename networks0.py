...
class rm_vsl_co(object):
    ...
    def __init__(self, test=False, visualization=False, control_horizon=60, incidents=False):
        ...
        self.inID = ['m3_0loop', 'm3_1loop', 'm3_2loop',
                     'm3_3loop', 'm3_4loop', 'rlight1_0loop']
        self.outID = ['m9_0loop', 'm9_1loop', 'm9_2loop',
                      'm9_3loop', 'm9_4loop', 'rout1_0loop']
        self.bottleneck_detector = [
            'm7_6loop', 'm7_7loop', 'm7_8loop', 'm7_9loop', 'm7_10loop', 'm7_11loop']
        ...
        if self.visualization == False:
            self.sumoBinary = r"C:\Program Files (x86)\Eclipse\Sumo\bin/sumo"
        else:
            self.sumoBinary = r"C:\Program Files (x86)\Eclipse\Sumo\bin/sumo-gui"
        ...
        if self.incidents == False:
            ...
            self.incident_time = np.random.randint(
                low=1, high=self.simulation_hour * 3600 - 1800)
    ...
    def writenewtrips(self):
        with open('map.rou.xml', 'w') as routes:
            ...
            for hours in range(len(self.m1flow)):
                ...
                vNum = m_in + r3_in
                dtime = np.random.uniform(
                    0+3600*hours, 3600+3600*hours, size=(int(vNum),))
                dtime.sort()
                for veh in range(int(vNum)):
                    typev = np.random.choice([0, 1, 2, 3], p=self.v_ratio)
                    vType = 'type' + str(typev)
                    route = np.random.choice(
                        [0, 1, 2], p=[m_in*self.m1a[0]/vNum, m_in*self.m1a[1]/vNum, r3_in/vNum])
                    routes.write("""<vehicle id=\"""" + str(temp+veh) + """\" depart=\"""" + str(round(dtime[veh], 2)) + """\" type=\"""" + str(
                        vType) + """\" route=\"""" + str(route) + """\" departLane=\""""'random'"""\"/>""" + '\n')
                    routes.write('\n')
                temp += vNum
            routes.write("""</routes>""")
        # reset incidents
    ...
    def run_step(self, v):
        ...
        for i in range(self.control_horizon):
            ...
            if self.simulation_step == self.incident_time:
                ...
                self.inci_edge = traci.vehicle.getRoadID(
                    self.inci_veh)  # get incident edge
            if self.simulation_step > self.incident_time and self.simulation_step < self.incident_time + self.incident_length:
                # set speed as zero, to simulate incidents
